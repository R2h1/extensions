...
interface SalStt{monthlyIncome:number;payDay:number}
...
async function getSal():Promise<SalStt>{const r=await chrome.storage.sync.get(SR);return r[SR] as SalStt??{monthlyIncome:10000,payDay:10};}
async function setSal(s:SalStt){await chrome.storage.sync.set({[SR]:s});}
const WDPM=21.75;
...
let rendered:Record<string,boolean>={},salStt:SalStt={monthlyIncome:10000,payDay:10};
...
async function renderSetSalary(){
  ...
  document.getElementById('sSalSave')!.addEventListener('click',async()=>{
    ...
    salStt={monthlyIncome:inc,payDay:d};await setSal(salStt);
    document.getElementById('sSalStat')!.textContent='已保存';document.getElementById('sSalStat')!.style.display='block';setTimeout(()=>document.getElementById('sSalStat')!.style.display='none',2500);
  });
}
...
function tickSalary(){
  const amt=document.getElementById('salAmount'),det=document.getElementById('salDetail'),tag=document.getElementById('salTag'),pdp=document.getElementById('salPayDay');
  if(!amt||!det||!tag)return;
  const n=new Date();
  const wStart=`09:00`,wEnd=`${pad(schedule.endHour)}:${pad(schedule.endMinute)}`;
  const [sh,sm]=wStart.split(':').map(Number),[eh,em]=wEnd.split(':').map(Number);
  const startDt=new Date(n.getFullYear(),n.getMonth(),n.getDate(),sh,sm,0);
  ...
  if(workSec<=0||!isWorkDay()){amt.textContent='0.00';det.textContent='非工作时间';tag.textContent='休息';tag.style.background='rgba(0,0,0,0.04)';tag.style.color='var(--text-tertiary)';}
  else{
    const now=n.getTime();
    if(now<startDt.getTime()){amt.textContent='0.00';det.textContent='待开工';tag.textContent='待开工';tag.style.background='rgba(0,0,0,0.04)';tag.style.color='var(--text-tertiary)';}
    else if(now>endDt.getTime()){const full=salStt.monthlyIncome/WDPM;amt.textContent=full.toFixed(2);det.textContent='今天已赚满';tag.textContent='下班';tag.style.background='rgba(0,0,0,0.04)';tag.style.color='var(--text-tertiary)';}
    else{
      const rate=salStt.monthlyIncome/WDPM/workSec,elapsed=Math.floor((now-startDt.getTime())/1000),acc=rate*elapsed;
      const h=Math.floor(elapsed/3600),m=Math.floor((elapsed%3600)/60);
      amt.textContent=acc.toFixed(2);det.textContent=`每秒 ${rate.toFixed(6)} 元 · 已工作 ${h}h${m}m`;
  ...
  if(pdp){
    ...
    pdp.textContent=diff===0?'今天发薪日  ':'距离发薪 · '+diff+' 天 · '+pad(next.getMonth()+1)+'月'+pad(salStt.payDay)+'日';
  }
}
function initSalary(){loadSal();tickSalary();}
...
async function init(){await loadSch();await loadM();await loadSal();renderAll();setInterval(()=>{updT();updC();tickSalary();},1000);}
init();